...
class ReleaseBuilder:
        ...
        if self.build_bat.exists():
            ...
            if spec_file.exists():
                return self.run_command(
                    f"pyinstaller --clean --noupx {spec_file}",
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                ...
                config_files = [
                    (
                        ...
                        "config.json",
                    ),  # Use template for clean config
                    (
                        "app/last_journal_scan.json.template",
                        "last_journal_scan.json",
                    ),  # Use template for clean scan state
                        ...
                        if item_path.is_dir():
                            ...
                            for file_path in item_path.rglob("*"):
                                if file_path.is_file():
                                    ...
                                    if (
                                        item == "Reports"
                                        and file_path.name == "sessions_index.csv"
                                    ):
                                        print(
                                            f"⏩ Skipped: {file_path.name} (test data)"
                                        )
                                        continue
                ...
                if doc_dir.exists():
                    for file_path in doc_dir.rglob("*"):
                        if file_path.is_file():
                            arc_name = (
                                f"EliteMining/Doc/{file_path.relative_to(doc_dir)}"
                            )
                            zipf.write(file_path, arc_name)
                ...
                galaxy_db_path = (
                    self.project_root / "app" / "data" / "galaxy_systems.db"
                )
                if galaxy_db_path.exists():
                    ...
                    print(
                        f"✅ Added: EliteMining/app/data/galaxy_systems.db ({size_mb:.1f} MB)"
                    )
                else:
                    print(
                        "⚠️  Galaxy systems database not found - ZIP users may need to download separately"
                    )
                ...
                if user_db_path.exists():
                    ...
                    print(
                        f"✅ Added: EliteMining/app/data/user_data.db ({size_kb:.1f} KB) - Pre-populated with hotspots"
                    )
                else:
                    print(
                        "⚠️  User database not found - ZIP users will start with empty database"
                    )

                # Add database metadata if it exists
                metadata_path = (
                    self.project_root / "app" / "data" / "database_metadata.json"
                )
    ...
    def run_installer_script(self):
        ...
        if not inno_compiler:
            ...
            return self.run_command(
                f'"{self.installer_iss}"', cwd=str(self.project_root)
            )
        else:
            ...
            if result and existing_installer.exists():
                ...
                new_mod_time = datetime.datetime.fromtimestamp(
                    existing_installer.stat().st_mtime
                )
