...
class EliteFileWatcher:
    ...
    def start_monitoring(self):
        ...
        with self._lock:
            ...
            if WATCHDOG_AVAILABLE and self.observer:
                try:
                    self.observer.schedule(self.event_handler, self.journal_dir, recursive=False)
                    self.observer.start()
    ...
    def _check_file_change(self, file_path: str):
        ...
        try:
            ...
            if (file_key not in self._file_sizes or 
                self._file_sizes[file_key] != current_size or
                file_key not in self._last_processed or
                current_mtime > self._last_processed[file_key]):
    ...
    def _handle_file_change(self, file_path: str):
        ...
        try:
            ...
            if file_name in self._last_processed:
                if current_time - self._last_processed[file_name] < self._debounce_delay:
                    return
